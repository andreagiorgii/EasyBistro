...
public class PrenotazioneValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		if (prenotazione.getOrario() != null) {
			if (!((prenotazione.getOrario().getHour() <= MAX_ORARIO_PRANZO
					&& prenotazione.getOrario().getHour() >= MIN_ORARIO_PRANZO)
					|| (prenotazione.getOrario().getHour() <= MAX_ORARIO_CENA
							&& prenotazione.getOrario().getHour() >= MIN_ORARIO_CENA))) {
	...
	// if(prenotazione.getUsers().getId().equals(loggedUser.getId()) &&
	// this.prenotazioneService.alreadyExists(prenotazione) != null) {
	// System.out.println(prenotazione.getUsers().getId().equals(loggedUser.getId()));
	// errors.rejectValue("dataPrenotazione","prenotazione.duplicato");
	// }

	// if(luogo.equals(Prenotazione.INTERNO_POSTO) &&
	// !prenotazione.isPrenotabileInterno(numeroPersone)) {
	// errors.rejectValue("numeroPersone","prenotazione.postiNonDisponibili");
	// }
	//
	// if(luogo.equals(Prenotazione.ESTERNO_POSTO) &&
	// !prenotazione.isPrenotabileEsterno(numeroPersone)) {
	// errors.rejectValue("numeroPersone","prenotazione.postiNonDisponibili");
	// }
